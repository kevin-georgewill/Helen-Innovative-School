  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-his-navy/95 backdrop-blur-md border-b border-white/10 shadow-lg'
          : 'bg-transparent'
      }`}
    >
      ...
      {menuOpen && (
        <div className="md:hidden bg-his-navy border-t border-white/10 px-4 py-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 pt-2 border-t border-white/10">
            <Link href="/login" onClick={() => setMenuOpen(false)} className="text-sm font-semibold text-white/80 py-2 text-center">
              Log in
            </Link>
            <Link href="/register" onClick={() => setMenuOpen(false)} className="bg-his-green hover:bg-his-green-hover text-white text-sm font-semibold px-6 py-2.5 rounded-full text-center transition-colors duration-150">
              Get Started
            </Link>
          </div>
        </div>
